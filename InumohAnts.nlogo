...
to setup-food
  ...
  ;; Reset food source counter
  ask patches [ set food-source-number 0 ]

  ;; Place new food based on selection
  if food-placement = "top-left" [
    setup-food-top-left
  ]
  if food-placement = "corners" [
    setup-food-corners
  ]
  if food-placement = "random" [
    setup-food-random
  ]
end

to setup-food-top-left
  ;; Place food sources uniformly on top-left side
  if num-food-sources = 2 [
    ask patch -20 20 [ setup-food-patch ]
    ask patch -10 20 [ setup-food-patch ]
  ]
  if num-food-sources = 3 [
    ask patch -25 20 [ setup-food-patch ]
    ask patch -15 20 [ setup-food-patch ]
    ask patch -5 20 [ setup-food-patch ]
  ]
end

to setup-food-corners
  ;; Place food in corners (clockwise from top-left)
  if num-food-sources = 2 [
    ask patch -25 25 [ setup-food-patch ]  ;; top-left
    ask patch 25 25 [ setup-food-patch ]   ;; top-right
  ]
  if num-food-sources = 3 [
    ask patch -25 25 [ setup-food-patch ]  ;; top-left
    ask patch 25 25 [ setup-food-patch ]   ;; top-right
    ask patch 25 -25 [ setup-food-patch ]  ;; bottom-right
  ]
end

to setup-food-random
  ;; Simple random placement with predefined safe locations
  let safe-locations (list
    patch -20 20
    patch 20 20
    patch -20 -20
    patch 20 -20
  ...
  set safe-locations shuffle safe-locations

  ;; Take the first num-food-sources locations
  let selected-locations sublist safe-locations 0 num-food-sources

  ;; Place food at selected locations
  foreach selected-locations [ loc ->
    ask loc [
      setup-food-patch
    ]
  ]
...
to setup-food-patch
  ;; Helper to setup food on a patch
  if food = 0 [
    set food-source-number food-source-number + 1
    set pcolor cyan
    set food 100  ;; Each source has 100 units of food
  ]
end
...
to go  ;; forever button
  ;; Stop if all food is collected
  if sum [food] of patches = 0 [
    stop
   ]
  ask turtles
